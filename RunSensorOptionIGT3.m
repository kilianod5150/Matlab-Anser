...
transformName3 = 'ProbeToTracker3';
...
if(IGTENABLE == 1)
    ...
    transform3.name = transformName3;
end
...
while (~FS.Stop())
   ...
   sys = fGetSensorPosition(sys, sensorsToTrack(3));
   disp(sys.positionVector);
   ...
   if(IGTENABLE == 1)
      ...
      sys.positionVectorMatrix = fSphericalToMatrix(sys.positionVector); 
      % Applies registration for the IGT coordinate system.      
      transform3.matrix = sys.registration * sys.positionVectorMatrix;
      % Generate a timestamp for the data and sent the transform through
      % the OpenIGTLink connection.
      transform3.timestamp = igtlTimestampNow();
      igtlSendTransform(slicerConnection, transform3);
   end
end
