...
function sys = fGetSensorPositionWithGain(sys, sensorNo)
...
positionVector = fSysDecodeFieldWithGain(sys);
% Convert the vector to a homogenous transformation matrix.
positionMatrix = fSphericalToMatrix(positionVector(1:5));

% Set the initial conditions for the next iteration to the current position
sys.estimateInit(sys.SensorNo,:) = positionVector(1:5);
sys.positionVector = positionVector(1:5);
