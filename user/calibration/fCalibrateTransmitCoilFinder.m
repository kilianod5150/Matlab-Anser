function sys = fCalibrateTransmitCoilFinder(sys)
% fCalibrate.m
...
options = optimset('TolFun',1e-22,'TolX',1e-8,'MaxFunEvals',2000,'MaxIter',2000,'Display','iter'); 
...
lowerLimit=[-.25 -.25 -.25 -5*pi -5*pi -5*pi -100 -5*pi -5*pi]; 
upperLimit=[.25 .25 .25 5*pi 5*pi 5*pi 100 5*pi 5*pi];
...
for coilNo = 1:numCoils
    ...
    paraEst=[x_offset(coilNo) y_offset(coilNo) 0 coil_flat_angle(coilNo) 0 0 1 0 0]
    ...
    f = @(params)objectiveTransmitCoilFinder(params, calFieldCoil, coilNo, sys);
